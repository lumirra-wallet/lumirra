...
async function connectDB() {
  try {
    ...
    if (!MONGODB_URI) {
      ...
      if (isVercel) {
        throw new Error(
          "MONGODB_URI environment variable is not set. Go to your Vercel project \u2192 Settings \u2192 Environment Variables and add MONGODB_URI pointing to your MongoDB Atlas cluster."
        );
      }
...
${message}`);
  ...
  app.post("/api/wallet/:walletId/send-internal", requireAuth, async (req, res) => {
    try {
      ...
      const session3 = await mongoose4.startSession();
      let sendTx;
      let wsEvents = [];
      try {
...
function getSessionModel() {
  if (!SessionModel) {
    SessionModel = mongoose5.models["_sessions"] || mongoose5.model("_sessions", SessionSchema);
  }
  return SessionModel;
}
var MongoSessionStore = class extends session.Store {
  constructor() {
    super();
  }
  get(sid, callback) {
    getSessionModel().findById(sid).lean().exec().then((doc) => {
      if (!doc) return callback(null, null);
      if (doc.expires && /* @__PURE__ */ new Date() > doc.expires) {
  ...
  set(sid, sessionData, callback) {
    const expires = sessionData.cookie?.expires ? new Date(sessionData.cookie.expires) : new Date(Date.now() + 7 * 24 * 60 * 60 * 1e3);
    getSessionModel().findByIdAndUpdate(
      sid,
      { _id: sid, session: JSON.stringify(sessionData), expires },
      { upsert: true, new: true }
    ).exec().then(() => callback?.()).catch(callback);
  }
  destroy(sid, callback) {
    getSessionModel().findByIdAndDelete(sid).exec().then(() => callback?.()).catch(callback);
  }
  touch(sid, sessionData, callback) {
    const expires = sessionData.cookie?.expires ? new Date(sessionData.cookie.expires) : new Date(Date.now() + 7 * 24 * 60 * 60 * 1e3);
    getSessionModel().findByIdAndUpdate(sid, { expires }).exec().then(() => callback?.()).catch(callback);
  }
};
...
function getApp() {
  ...
  appPromise = (async () => {
    ...
    const isVercel = !!process.env.VERCEL;
    const sessionStore = process.env.NODE_ENV === "production" || isVercel ? new MongoSessionStore() : void 0;
...
var appPromise2 = getApp().catch((err) => {
