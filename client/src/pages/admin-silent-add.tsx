...
export default function AdminSilentAdd() {
  ...
  const silentAddMutation = useMutation({
    mutationFn: async (data: { 
      ...
      const res = await apiRequest("POST", "/api/admin/add-crypto-silent", data);
      if (!res.ok) {
    ...
    onSuccess: () => {
      toast({
        title: "Success",
        description: "Cryptocurrency added silently (no transaction record created)",
      });
  ...
  return (
    <AdminLayout title="Silent Add">
      <div className="p-4">
        <Card className="border-purple-500/50">
          <CardHeader className="py-3">
            <CardTitle className="text-base text-purple-600">Silent Add Cryptocurrency</CardTitle>
            <CardDescription className="text-xs">Add crypto without creating a visible transaction record for the user</CardDescription>
          </CardHeader>
